...
void Utunes::handle_recommended(ostringstream &body){
	update_users_and_songs_matrix();
	update_similarity_matrix();
	...
	for(int i=0; i<recom_vector.size(); i++){
		...
		body
		<<"<tr>"<<endl
		<<" <td>"<<songs[song_id-1].get_title()<<"</td>"<<endl
		<<" <td>"<<songs[song_id-1].get_artist()<<"</td>"<<endl
		<<" <td>"<<songs[song_id-1].get_released_year()<<"</td>"<<endl
		<<"</tr>"<<endl;
	}
}

ostringstream Utunes::make_songs_body(){
	ostringstream body;
	body
	<<"<!DOCTYPE html>"<<endl
	<<"<html>"<<endl
	...
	<<"</head>"<<endl
	<<"<body>"<<endl
	<<"<h2 style=\"text-align: center;\">Songs</h2>"<<endl
	<<"<p style=\"text-align: center;\"><img src=\"/utuneslogo.png\
	\" style=\"width:25%;\"></p>" << endl
	<<"<table>"<<endl
 	<<"<tr>"<<endl
 	<<"  <th>ID</th>"<<endl
  	<<"  <th>Title</th>"<<endl
	...
	for(int i=0; i<songs.size(); i++){
		...
		<<" <td><a href='song_info?id="<<songs[i].get_id()<<"'>"
		<<songs[i].get_title()<<"</a></td>"<<endl
    	<<" <td>"<<songs[i].get_artist()<<"</td>"<<endl
    	<<" <td>"<<songs[i].get_released_year()<<"</td>"<<endl
		<<"</tr>"<<endl;
	}
	body
...
ostringstream Utunes::make_song_info_body(int id_){
	int id = id_;
	ostringstream body;
	body
	<<"<!DOCTYPE html>"<<endl
	...
	<<"</head>"<<endl
	<<"<body>"<<endl
	<<"<h2 style=\"text-align: center;\">Song Info</h2>"<<endl
	<<"<p style=\"text-align: center;\"><img src=\"/utuneslogo.png\
	\" style=\"width:25%;\"></p>" << endl
	<<"<table>"<<endl;
	body
	<<"<tr>"<<endl
	<<" <td>Title: </td>"<<endl
	<<" <td>"<<songs[id-1].get_title()<<"</td>"<<endl
	<<"</tr>"<<endl
	<<"<tr>"<<endl
	<<" <td>Artist: </td>"<<endl
	<<" <td>"<<songs[id-1].get_artist()<<"</td>"<<endl
	<<"</tr>"<<endl
	<<"<tr>"<<endl
	<<" <td>Released Year: </td>"<<endl
	<<" <td>"<<songs[id-1].get_released_year()<<"</td>"<<endl
	<<"</tr>"<<endl
	<<"<tr>"<<endl
	<<" <td>Number of Likes: </td>"<<endl
	<<" <td>"<<songs[id-1].get_num_of_likes()<<"</td>"<<endl
	<<"</tr>"<<endl
	<<"<tr>"<<endl
	<<" <td>Number of Playlists: </td>"<<endl
	<<" <td>"<<songs[id-1].get_num_of_playlists()<<"</td>"<<endl
	<<"</tr>"<<endl;
	body
	<< "<a href=\"/logout\">logout</a>" << endl
	<< "   <br />" << endl
	<< "   <br />" << endl
	<<"<audio controls>"
	<<"  <source src="<<songs[id-1].get_link()<<" type=\"audio/mpeg\">"
	<<"</audio>"<<endl;

	handle_recommended(body);
	body<<"</body>";
	return body;
}
